...
f= lambda x: a*np.sin(np.pi*x) #this will update when I change a
...
os.chdir(dirname)
os.system('ffmpeg -r %d -i %s.jpg -vcodec libx264 -y %s.mp4 -qscale 0' %(int(1.0/frame_duration),'%04d',dirname))
